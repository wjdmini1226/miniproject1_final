...
insert into member values(seq_member.nextVal,'admin','1234','admin',2);

ALTER TABLE board ADD (
  b_is_notice   CHAR(1) DEFAULT 'N' NOT NULL,   -- 공지 여부(Y/N)
  b_is_ad       CHAR(1) DEFAULT 'N' NOT NULL    -- 홍보글 여부(Y/N) (사장)
);

-- 공지 상단 고정을 위해 정렬용 인덱스(선택)
CREATE INDEX idx_board_notice ON board(b_is_notice, b_idx);
